...
def process_state_law_pdf(pdf_path, state_name, output_dir="state_law_jsons"):
    ...
    safe_print(f"Processing {state_name} law from {pdf_path}...")
    
    # Extract text
    raw_text = extract_text_from_pdf(pdf_path)
    safe_print(f"Extracted {len(raw_text)} characters from {pdf_path}")
    
    # Parse text into structured format
    formatted_sections = parse_state_law_text(raw_text, state_name)
    safe_print(f"Parsed into {len(formatted_sections)} sections")
    
    # Show sample of what we parsed
    if formatted_sections:
        safe_print(f"Sample section: {formatted_sections[0]['section']} - {formatted_sections[0]['title']}")
        safe_print(f"Chapter: {formatted_sections[0]['chapter']}")
        preview = formatted_sections[0]['text'][:200].replace('\n', ' ')
        safe_print(f"Text preview: {preview}...")
    ...
    safe_print(f"Saved formatted law to {output_file}")
    return output_file
    ...
    for pdf_file, state_name in state_pdfs:
        ...
        if os.path.exists(pdf_path):
            try:
                ...
                safe_print(f"SUCCESS: Processed {state_name}\n")
            except Exception as e:
                safe_print(f"ERROR: Processing {state_name}: {str(e)}\n")
        else:
            safe_print(f"WARNING: File not found: {pdf_file}")
